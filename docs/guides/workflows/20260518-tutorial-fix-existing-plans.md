...
**Worked example in this repository:** `{ITERATION_CARRIER}` = `.work/plans/NEXT.md`, `{MASTER_PLAN}` = `.work/plans/full/20260517-full-plan.md` (sections **19–21** are the usual anchors).

---

## 1. Know the three layers (do not conflate them)

| Layer | Typical file (this repo) | What it must contain |
|-------|--------------------------|----------------------|
...
2. **Section numbers** — In this repo, **19** = incremental roadmap (milestones + task tables), **20** = global acceptance, **21** = validation gates. Your plan may differ; **record the section ids** in `{HANDOFF}` so agents stop guessing.  
...
### Optional improvement for this repo
...
1. Pick the **SPEC** that owns the behaviour (e.g. `fiscal-pipeline` SPEC).  
...
2. For each milestone that touches HTTP, jobs, or logging: ensure **M1-style** platform tasks exist **before** domain milestones emit user traffic (this repo: `M1-T4`, `M1-T8` already encode that intent).  
