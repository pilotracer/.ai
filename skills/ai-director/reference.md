...
| `session-control` | `session-control/` | Session bookends; updates HANDOFF + NEXT; optional git | `start`, `close`, `close commit`, `close commit push`, `status` | Yes (`{HANDOFF}`, `{ITERATION_CARRIER}`, optional git) | Recommended: `@project-bootstrap init` |
