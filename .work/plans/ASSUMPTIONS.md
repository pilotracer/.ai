# ASSUMPTIONS — planning registry

> **This is a template.** In a real adopter repo this file is maintained by **`@plan-foundation`** and **`@plan-master`**. In this framework repo it stays as a demo skeleton.

**Updated:** YYYY-MM-DD · **Maintained by:** plan-foundation / plan-master

Label every entry: **Confirmed** | **Inference** | **Unverified** | **Rejected**

| ID | Assumption | Label | Source | Notes |
|----|------------|-------|--------|-------|
| A1 | REPLACE:PROJECT_NAME targets REPLACE:PRIMARY_USER_SEGMENT | Unverified | foundation doc 01 | |
| A2 | Dev workflow follows `.cursorrules` (Docker or documented local CI) | Confirmed | `.cursorrules` | |

## Rejected

| ID | Assumption | Reason |
|----|------------|--------|
| — | (none) | |

## Review log

| Date | Reviewer | Action |
|------|----------|--------|
| YYYY-MM-DD | bootstrap | Initial template |
